...
class MGM(Agent):
    ...
    def decide(self) -> None:
        ...
        if self.mode == "select":
            lrs = {m.sender: float(m.data[0]) for m in self.inbox if m.msg_type == "lr"}
            # check if this agent has the highest lr
            highest_lr = True
            for n, lr in lrs.items():
                if lr > self.best_lr:
                    highest_lr = False
                    break
                elif lr == self.best_lr and n < self.name:
                    highest_lr = False
                    break

            if highest_lr and self.best_lr>0:
